...
def hyd_model(
    ...
    wb = (_storage_1 - storage_1) + (_storage_2 - storage_2) + precip - E - Q
...
input_csv = snakemake.input.weather_csv
output_csv = os.path.abspath(snakemake.output.hydro_csv)

params = snakemake.params.hydro_parameters
...
for n, r in df.iterrows():
    ...
    storage_1, storage_2, Q, E, wb = hyd_model(
        storage_1, storage_2, precip, pot_evapo, **params
    )
    Qs.append(Q)
    Es.append(E)
    wbs.append(wb)
...
df = pd.DataFrame({"Q": Qs, "E": Es, "wb": wbs}, index=df.index)
df.index.name = "date"
